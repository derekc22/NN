...
from src.recurrent_cell_copy import RecurrentCell
...
class RNN(Network):
    ...
    # def forward_deprecated(self, X, training):
    ...
    def forward(self, X, training):
        ...
        ht1_l = [layer.ht1 for layer in self.layers[:-1]] if self.stateful else self.zero_state()
        whh_l = [layer.whh for layer in self.layers[:-1]]
        wxh_l = [layer.wxh for layer in self.layers]
        bh_l  = [layer.bh  for layer in self.layers[:-1]]
        why = self.layers[-1].wxh
